/* ================================================================
   app.js — UI コンポーネント
   ★ デザイン変更のみここを編集
   ★ data.js の prefectures / descriptions を参照
   ================================================================ */
  ...
  return (
    <div style={{
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
        ...
        <div onClick={onClose} style={{padding:'2px 8px',fontSize:'18px',cursor:'pointer',color:C.mute,lineHeight:1}}>×</div>
      </div>
      {text ? (
        <>
...
function ListView({ onSelect, favorites, onToggleFav, search, setSearch, region, setRegion }) {
  const filtered = useMemo(() => {
    ...
    return prefectures.filter(p => {
      ...
      if (!q) return true;
      return [p.name,p.reading,p.capital,p.region,...p.food,...p.specialties,...p.nature,...(p.old_kuni||[]),...p.tags]
  ...
  return (
    <div style={{backgroundColor:C.bg, minHeight:'100vh'}}>
      ...
      <div style={{backgroundColor:C.paper, borderBottom:`1px solid ${C.border}`, padding:'16px'}}>
        ...
        <div style={{fontSize:'11px',color:C.mute,letterSpacing:'0.1em'}}>タップで詳細・解説を表示</div>
      </div>
      ...
      <div style={{overflowX:'auto', whiteSpace:'nowrap', padding:'8px 16px', backgroundColor:C.paper, borderBottom:`1px solid ${C.border}`}}>
        {regions.map(r => (
          <span key={r} onClick={() => setRegion(r)} style={{
            display:'inline-block', marginRight:'6px',
            padding:'5px 12px', fontSize:'12px', borderRadius:'20px', cursor:'pointer', userSelect:'none',
      ...
      {/* 件数 */}
      <div style={{padding:'8px 16px', fontSize:'12px', color:C.mute}}>{filtered.length}件表示</div>
      ...
      <div style={{padding:'0 12px 24px', display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:'10px'}}>
        {filtered.map(p => (
          <div key={p.id} onClick={() => onSelect(p)} style={{
            ...
            cursor:'pointer', userSelect:'none', borderLeft:`3px solid ${C.red}`,
          }}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              ...
              <div onClick={e => {e.stopPropagation(); onToggleFav(p.id);}} style={{padding:'4px',cursor:'pointer',fontSize:'16px',lineHeight:1}}>
                {favorites.has(p.id) ? '★' : '☆'}
              </div>
            </div>
            <div style={{marginTop:'8px',fontSize:'11px',color:C.soft}}>
              <span style={{color:C.red,fontSize:'10px'}}>{p.region}</span>{' · '}{p.capital}
            </div>
  ...
  return (
    <div style={{backgroundColor:C.bg, minHeight:'100vh'}}>
      ...
      <div onClick={onBack} style={{
        backgroundColor:C.ink, color:C.paper, padding:'14px 16px', fontSize:'15px',
        cursor:'pointer', display:'flex', alignItems:'center', gap:'8px', userSelect:'none',
      }}>
        <span style={{fontSize:'20px'}}>←</span><span>一覧に戻る</span>
      </div>
      ...
      <div style={{backgroundColor:C.paper, padding:'16px', borderBottom:`1px solid ${C.border}`}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          ...
          <div onClick={() => onToggleFav(pref.id)} style={{padding:'8px',fontSize:'24px',cursor:'pointer',lineHeight:1}}>
            {isFavorite ? '★' : '☆'}
          </div>
        </div>
        ...
        <div style={{marginTop:'8px',fontSize:'11px',color:C.green}}>▼ 各項目をタップすると解説が表示されます</div>
      </div>

      {/* 解説ボックス */}
      {selected && (
        <div style={{padding:'8px 16px'}}>
          <DescBox item={selected} onClose={() => setSelected(null)} />
        </div>
      )}
...
function JapanPrefecturesApp() {
  ...
  const handleSelect = (pref) => { setSelectedPref(pref); window.scrollTo(0,0); };
  const handleBack   = ()     => { setSelectedPref(null); window.scrollTo(0,0); };
...
// マウント
ReactDOM.createRoot(document.getElementById('root')).render(<JapanPrefecturesApp />);
